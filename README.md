...
- Clone the repository.
    ```
    git clone https://github.com/shanosha/mod-14-lab-2.git
    ```
...
- **Create Note:** POST /
  - Creates a new note using the data in req.body.
  - Required JSON Properties: title, content
  - Requires: JWT token in a Bearer header
- **Read All Notes:** GET /
  - Retrieves all notes from the database.
  - Requires: JWT token in a Bearer header
- **Read One Note:** GET /:id
  - Retrieves a single note by its _id.
  - Requires: JWT token in a Bearer header
- **Update Note:** PUT /:id
  - Updates a note by its _id using the data in req.body.
  - Requires: JWT token in a Bearer header
- **Delete Note:** DELETE /:id
  - Deletes a note by its _id.
  - Requires: JWT token in a Bearer header